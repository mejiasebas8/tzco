...
const ParticleFlower = () => {
  ...
  useEffect(() => {
    ...
    function initializeParticles() {
      ...
      // Updated to use a smaller particle count for better mobile performance
      ...
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        const r = Math.pow(Math.random(), 0.5) * formScale * 0.5 * (baseSize/4);
    ...
    function animate() {
      ...
      // Updated time increment for faster animation
      timeRef.current += 0.003;
      
      // Clear with slight trails for ghosting effect
      ctx.fillStyle = 'rgba(0, 0, 0, 0.08)'; // Increased opacity for better trails
      ...
      particlesRef.current.forEach(particle => {
        ...
        const dist = Math.sqrt(dx * dx + dy * dy) / (baseSize/4);
        ...
        const size = Math.max(0.001, 0.6 * depthFactor * (baseSize / 550));
        
        ctx.beginPath();
        ctx.arc(particle.x, particle.y, size, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(248, 246, 240, ${opacity})`;
        ctx.fill();
      });
